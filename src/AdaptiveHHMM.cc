                        ...
                        const tensor_d& old_trans_mats, matrix_d & gammas)
{
    ...
    matrix_d hidden_sums(N, vector_d(H, 0));
    tensor_d hidden_mat(N, matrix_d(H, vector_d(H, 0)));
    tensor_d trans_mats(H, matrix_d(N, vector_d(N, 0)));    

    for (int t = 0; t < T-1; t++)
    {   
        tensor_d aux_trans_mats(H, matrix_d(N, vector_d(N, 0)));
        tensor_d aux_hidden_mat(N, matrix_d(H, vector_d(H, 0)));
        ...
        for (int h = 0; h < H; ++h){
            for(int g = 0; g < H; ++g){
                ...
                for(int i = 0; i < N; ++i) {
                    for(int j = 0; j < N; ++j) {
                        // Revisar
                        double curr_epsilon = forw[t][g][i]*old_hidden_mat[i][h][g]*old_trans_mats[h][j][i]*
                                              distrib[t+1][j]*back[t+1][h][j];
                        aux_trans_mats[h][j][i] += curr_epsilon;  
                        aux_hidden_mat[i][h][g] += curr_epsilon;
        ...
        for (int h = 0; h < H; ++h){
            for(int i = 0; i < N; ++i){
                ...
                for (int g = 0; g < H; ++g){
                    double update = aux_hidden_mat[i][h][g]/total_it_sum;
                    hidden_mat[i][h][g] += update;
                    hidden_sums[i][g] += update;
                }
            }
        }
        for (int i = 0; i < N; ++i)
    ...
    for (int h = 0; h < H; ++h){
        for (int i = 0; i < N; ++i){
            for (int j = 0; j < N; ++j) {
                ...
                if (trans_mats[h][j][i] < 1e-10)
                    trans_mats[h][j][i] = 0;
            }
            for (int g = 0; g < H; ++g) {
                hidden_mat[i][h][g] /= hidden_sums[i][g];
                if (hidden_mat[i][h][g] < 1e-10)
                    hidden_mat[i][h][g] = 0;
            }
        }
    }
                        ...
                        const tensor_d& old_trans_mats)
{
    ...
    {   
        ...
        for (int h = 0; h < H; ++h){
            for(int g = 0; g < H; ++g){
                // Hidden_update is epsilon(t)
                double curr_epsilon = forw[t][g]*old_hidden_mat[i][h][g]*
    ...
    for (int h = 0; h < H; ++h){
        for (int i = 0; i < N; ++i){
            ...
            for (int g = 0; g < H; ++g) {
                hidden_mat[i][h][g] /= hidden_sums[i][g] + eps;
                if (hidden_mat[i][h][g] < 1e-10)
                    hidden_mat[i][h][g] = 0;
            }
        }
                    ...
                    const int iter_print)
{
    ...
    vector<vector<vector<pair<int, int>>>> prev_state(T, 
           vector<vector<pair<int, int>>>(H, vector<pair<int, int>>(S)));
    ...
    for(int h = 0; h < H; ++h){
        for (int i = 0; i < S; ++i){
            ...
            for(int g = 0; g < H; ++g){
                for(int j = 0; j < S; ++j){
                    double path_prob = initial_distr[g][j]*hidden_mat[j][h][g]
                                      *matrices[h][i][j]* distr[0][i];
                    highest_prob = (path_prob > highest_prob) ? path_prob : highest_prob;
                }
            }
            curr_prob[h][i] = highest_prob;
            sum += curr_prob[h][i];
        }
    }
    ...
    for (int t = 1; t < T; ++t)
    {
        ...
        for (int h = 0; h < H; ++h){
            for(int i = 0; i < S; ++i){
                ...
                for(int g = 0; g < H; ++g){
                    for(int j = 0; j < S; ++j){
                        double path_prob = curr_prob[g][j]*hidden_mat[j][h][g]*matrices[h][i][j]*distr[t][i];
    ...
    matrix_int decoded_all(2, vector_int(T, 0));
    
    decoded_all[0][T-1] = final_max.second;
    decoded_all[1][T-1] = final_max.first;
    
    for (int t = T-2; t >= 0; --t){
        int prev_s = decoded_all[0][t+1];
        int prev_h = decoded_all[1][t+1];
        
        decoded_all[0][t] = prev_state[t+1][prev_h][prev_s].second;
        decoded_all[1][t] = prev_state[t+1][prev_h][prev_s].first;
    }
                        ...
                        const tensor_d& hidden_mat, tensor_d& forw)
{
    ...
    for (int h = 0; h < H; ++h){
        for(int i = 0; i < N; ++i){
            ...
            for(int g = 0; g < H; ++g){
                for(int j = 0; j < N; ++j){
                    current_prob += init[g][j]*hidden_mat[j][h][g]*mats[h][i][j]*distr[0][i];
                }
            }
    ...
    for (int t = 1; t < T; t++){
        ...
        for (int h = 0; h < H; ++h){
            for (int i = 0; i < N; ++i){
                ...
                for (int g = 0; g < H; ++g){
                    for (int j = 0; j < N; ++j){
                        curr_prob += hidden_mat[j][h][g]*mats[h][i][j]*forw[t-1][g][j];
                    }
                }
                curr_prob *= distr[t][i];
                        ...
                        const tensor_d& hidden_mat, matrix_d& forw)
{
    ...
    for (int i = states[0], h = 0; h < H; ++h){
        ...
        for(int g = 0; g < H; ++g){
            for(int j = 0; j < N; ++j){
                current_prob += init[g][j]*hidden_mat[j][h][g]*mats[h][i][j];
            }
        }
    ...
    for (int t = 1; t < T; t++){
        ...
        for (int h = 0; h < H; ++h){
            ...
            double curr_prob = 0;
            for (int g = 0; g < H; ++g){
                curr_prob += forw[t-1][g]*hidden_mat[j][h][g]*mats[h][i][j];
            }
            forw[t][h] = curr_prob;
            curr_sum += curr_prob;
        }
    ...
    for (int t = T-2; t >=0; t--){
        ...
        for (int h = 0; h < H; ++h){
            for (int i = 0; i < N; ++i){
                ...
                for (int g = 0; g < H; ++g){
                    for (int j = 0; j < N; ++j){
                        curr_prob += hidden_mat[i][g][h]*mats[g][j][i]*back[t+1][g][j]*distr[t+1][j];
                    }
                }
...
void AdaptiveHHMM::backward_states(const vector_d &signal, const vector_int &states,
    ...
    for (int t = T-2; t >=0; t--){
        ...
        for (int h = 0; h < H; ++h){
            ...
            double curr_prob = 0;
            for (int g = 0; g < H; ++g){
                curr_prob += hidden_mat[i][g][h]*mats[g][j][i]*back[t+1][g];
            }
            back[t][h] = curr_prob;
            curr_sum += curr_prob;
        }
...
void AdaptiveHHMM::EM_maximization(const vector_d &signal, const vector_int &states,
    ...
    if (get_log_em()) {
            ...
            for (int i = 0; i < N; ++i){
                cout << "Matriu Hidden inicial " << i << endl;
                print(get_hidden_mat(i));
            }
            cout << "CHANNELS:" << endl;
            for(int h = 0; h < H; ++h){
    ...
    for (int it=0; abs(eps) > tol and it < maxits; ++it){
        ...
        tensor_d old_hidden = get_hidden_mat();
