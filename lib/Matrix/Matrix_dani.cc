...
// Intent de fer la classe
/*

matrix_d matrix_d::operator+(const double &left)
{
    int N = right.size();
...
matrix_d matrix_d::operator+(const matrix_d &right)
{
    int N = mat.size(),
        N2 = mat[0].size(),
        M = right.size(),
...
matrix_d matrix_d::operator-(const double &right)
{
    int N = mat.size();
...
matrix_d matrix_d::operator &(const matrix_d &right)
{
	...
	for (int i = 0; i < k1; ++i){
		for (int j = 0; j < k2; ++j){
			ans[i][j]=0;
			for (int z = 0; z < k3; ++z) ans[i][j] += mat[i][z]*right[z][j]; 		
		}
	}
	return ans;
}
...
/*
vector_d matrix_d::operator&(const vector_d &right)
{
    int n = right.size();
    int m1 = mat.size();
    int m2 = mat[0].size();
...
vector_d matrix_d::operator[](const int& i){
    assert(i < mat.size());
    return mat[i];
}

matrix_d::matrix_d();
matrix_d::matrix_d(int N, const vector_d& row):mat(N, row){};
int matrix_d::size(){
    return mat.size();
}
*/